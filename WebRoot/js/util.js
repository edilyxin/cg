 ...
 *	打开图标弹出窗口
 *	@param id 图标显示ID
 *	@param inputid 图标地址ID
 */
function openIconWindow(id,inputid) {
	var url="right!findImgList";
	var returnValue = openWindow(url,'','','');
	if (returnValue != undefined) {
		document.getElementById(inputid).value = returnValue;
		if(returnValue==""){
			document.getElementById(id).innerHTML = returnValue;
		}else{
			document.getElementById(id).innerHTML = "<img alt=\"集成图标\" style=\"border:1px #666666 solid;\" width=\"20\" height=\"20\"  align=\"middle\" src=\""+returnValue+"\"/>&nbsp;";
		}
	}
}
 ...
 */
function openWindow(url,obj,width,height) {
	...
	var returnValue = window.showModalDialog(url,obj,'dialogHeight:'+height+';dialogWidth:'+width+';center:yes;help:no;resizable:no;status:no;scroll:auto;location:no;');
	return returnValue;
}
 ...
 *  @param value：标签初始值
 *  @param url：验证地址
 *  @param obj：提交数据，例{id1:$("#id1").val(), id2:$("#id2").val(),id3:$("#id3").val()}
 *  @param errorID：错误提示ID
 *  @param errorMessage：错误提示信息
 *	@return	returnValue：返回值
 */
function checkValue(id,value,url,obj,errorID,errorMessage){
	var flag = false; 
	if(document.getElementById(id).value.trim()==""||document.getElementById(id).value.trim()==value){
		$('#'+errorID).html("");
		return true;
	}else{
		$('#'+errorID).html("正在检验中,请稍后……"); 
	}
	$.ajax({
		async:false,//同步
		cache:false,//不读取缓存信息
...
//		alert("不准你使用ALT+方向键前进或后退网页！");   
	...
	event.returnValue=forbidBackSpace();
	if (
