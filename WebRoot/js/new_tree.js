...
var rootIcon       = path+"images/tree_img/root_close.gif"; 	//根目录关起时状态图标
var openRootIcon   = path+"images/tree_img/root_open.gif"; 	//根目录展开时状态图标
var folderIcon     = path+"images/tree_img/folder_close.gif";	//子文件夹关起时状态图标
var openFolderIcon = path+"images/tree_img/folder_open.gif";	//子文件夹展开时状态图标
var fileIcon       = path+"images/iGenericAttribute.gif";	//链接叶子节点图标
var iIcon          = path+"images/tree_img/i_line.gif";		//普通的连接线（直的）
var lIcon          = path+"images/tree_img/l_line.gif";		//最后一个节点的连接线
var lMinusIcon     = path+"images/tree_img/l_point_open.gif";	//最后一个文件夹节点（展开时的）图标
var lPlusIcon      = path+"images/tree_img/l_point_close.gif";//最后一个文件夹节点（关起时的）图标
var tIcon          = path+"images/tree_img/t_line.gif";		//中间节点的连接线
var tMinusIcon     = path+"images/tree_img/t_point_open.gif";	//中间文件夹节点（展开时的）图标
var tPlusIcon      = path+"images/tree_img/t_point_close.gif";//中间文件夹节点（关起时的）图标
var blankIcon      = path+"images/tree_img/blank.gif";		//中间空白的图标图标
 ...
 * @returns {WebFXTree}
 */
function WebFXTree(sText, url,listname) {
	this._subItems = [];
...
//	str += "</td><td valign=\"bottom\" nowrap>&nbsp; <onclick=\"treeitem_Click('" + this.id + "');\" onDblClick=\"treeitem_DblClick('" + this.action + "','"+this.text+"');\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\">" + this.text + "</td></tr></table></div>";
	str += "</td><td valign=\"bottom\" nowrap>&nbsp;<a href=\"javascript:void(0);\" onclick=\"find('"+this.listname+"','"+this.url+"')\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\">" + this.text + "</a></td></tr></table></div>";
...
function WebFXTreeItem(sText, url,listname,showImg) {
	this._subItems  = [];
	this._wasLast   = false;
	this.text       = sText || defaultText;
//	this.action     = sAction || defaultAction;
...
//		str += "<onclick=\"treeitem_Click('" + this.id +  "','" + this.text + "','" + this.treeID + "','" + this.nodeId + "');\" onDblClick=\"change(this);\" id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" title=\""+this.text+"\">" + this.text;
		 str += "<a href=\"javascript:void(0);\" onclick=\"find('"+this.listname+"','"+this.url+"')\"  id=\"" + this.id + "-anchor\" onfocus=\"webFXTreeHandler.focus(this);\" title=\""+this.text+"\">" + this.text + "</a>"; 
...
function find(name,url){
	if(url==null||url==undefined||url=="undefined"||url=="")return false;
	if(name!=undefined&&name!=""){
		var nameList = name.split("|");
		for(var i=0;i<nameList.length;i++){
			var id = nameList[i].split("=")[0];
			var val = nameList[i].split("=")[1];
