...
KindEditor.plugin('filemanager', function(K) {
	var self = this, name = 'filemanager',
		filerJson = K.undef(self.filerJson, self.basePath + 'php/file_manager_json.php'),
	...
	self.plugin.filemanagerDialog = function(options) {
		...
		function reloadPage(path, order, func) {
			...
			K.ajax(K.addParam(filerJson, param + '&' + new Date().getTime()), function(data) {
				dialog.hideLoading();
				func(data);
			});
		}
...
KindEditor.plugin('flash', function(K) {
	var self = this, name = 'flash', lang = self.lang(name + '.'),
		...
		allowFiler = K.undef(self.allowFiler, false),
...
KindEditor.plugin('image', function(K) {
	...
	self.plugin.imageDialog = function(options) {
		...
		var dialogWidth = showLocal || allowFiler ? 450 : 400,
