			...
			<table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" bgcolor="#93B3CA" class="tabletext">
									     ...
									     String id = "";
									     		for(int i=0;i<li.size();i++){	   			
									     			 ...
									     			 if(sys.getSysint_sno().equals(sr.getSysint_sno())&&sr.getRight_sparentno().equalsIgnoreCase("0")){//系统oneRIGHT_SPARENTNO				     			 	
									     			 	id = sr.getRight_sno();
									     			 	%>
									     			 		var node<%=id%>= new WebFXTreeItem("<%=sr.getRight_sname() %>","<%=sr.getRight_sno() %>","0","<%=sr.getRight_sno() %>|<%=sys.getSysint_sno()%>","","1");		
									     			 ...
									     			 //第三层    取大功能下的小功能sr.getRight_sno().equals(sr.getRight_sparentno())&&
									     			 	...
									     			 	%>
									     			 		var node<%=sr.getRight_sno()%>= new WebFXTreeItem("<%=sr.getRight_sname() %>","<%=sr.getRight_sno() %>","0","<%=sr.getRight_sno() %>|<%=sys.getSysint_sno()%>","","1");	
    				...
    				<table width="100%" border="0" cellspacing="0" cellpadding="0">
      				<tr>
        				<td  colspan="5" nowrap>
        					<div align="center">
        					  <input type="button" value="保存" class="button" onclick="javascript:Multi_onClick('1');"/>&nbsp;
        					  <input type="button" value="进入下一步操作权限分配" class="button" onclick="javascript:Multi_onClick('2');"/>&nbsp;
